...
С таким способ представления, доступен следующий алгоритм:
...
* Если встречаем мутацию, которая является надмножеством текущей, то уменьшаем скоуп мутации до нее.
...
* Если массив freeze является подмассиовм мутации, то было изменение frozen элемента и мы выводим предупреждение.  
...
* Так как он базовый для `[c, foo, i]`, то теперь рассматриваем `[c, foo]`   
